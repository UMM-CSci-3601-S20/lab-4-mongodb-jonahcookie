...
public class TodoController {
  ...
  public void addNewTodo(Context ctx) {
    Todo newTodo = ctx.bodyValidator(Todo.class)
      .check((tod) -> tod.owner != null && tod.owner.length() > 0) //Verify that the todo has a name that is not blank
      .check((tod) -> tod.status == false || true) //Verify that the todo has a status
      .check((tod) -> tod.category != null && tod.category.length() > 0) // Verify that the todo has a category that is not blank (exists)
      .check((tod) -> tod.body != null && tod.body.length() > 0) //Verify that the todo has a body
      .get();
